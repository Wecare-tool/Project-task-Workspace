...
const formFields: FormField[] = [
    { name: 'name', label: 'Tên loại hành động', type: 'text', placeholder: 'VD: Gửi email', required: true },
    { name: 'code', label: 'Mã', type: 'text', placeholder: 'VD: SEND_EMAIL', required: true },
...
export function ActionTypePage() {
    const { actionTypeNews, isLoading } = useDataverse();
        ...
        <div className="space-y-2 animate-fade-in">
            ...
            <div className="card p-3">
                <DataTable<ActionTypeNew>
                    ...
                    emptyMessage={searchQuery ? "Không tìm thấy kết quả phù hợp" : "Chưa có loại hành động nào"}
                />
            </div>

            {/* Modals */}
            <Modal isOpen={isFormOpen} onClose={() => { setIsFormOpen(false); setSelected(null); }} title={selected ? 'Chỉnh sửa' : 'Thêm loại hành động'} size="md">
            ...
            <ConfirmModal isOpen={!!deleteItem} onClose={() => setDeleteItem(null)}
                ...
                title="Xóa loại hành động" message={`Xóa "${deleteItem?.name}"?`} isLoading={isLoading} />
        </div>
    );
}
